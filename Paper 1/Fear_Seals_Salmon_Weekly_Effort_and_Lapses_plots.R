...
# Gaps Lapses
read.gaps <- read.csv("ParameterManipulations/Outputs/Reps_Fear_Effort_Lapses_range_gap.csv")
colnames(read.gaps) <- c("Iteration", "gaps")
gaps_range <- read.gaps$gaps

read.eaten <- read.csv("ParameterManipulations/Outputs/Reps_Fear_Effort_Lapses_salmon_eaten_output.csv")
colnames(read.eaten) <- c("Iteration", "Eaten")
read.eaten$Value <- gaps_range
read.eaten$perc.Eaten <- read.eaten$Eaten / 10000 * 100

eaten_plot_gaps <- plotEaten(read.eaten, "Gap Days = (1 - 21)", c(0, 40), NULL)
...
read.seals.base <- read.csv("ParameterManipulations/Outputs/Reps_Fear_Effort_Lapses_gauntlet_seals_output.csv")
...
read.effort <- read.csv("ParameterManipulations/Outputs/Reps_Fear_Weekly_Effort_range_days.csv")
colnames(read.effort) <- c("Iteration", "effort")
effort_range <- read.effort$effort

read.eaten <- read.csv("ParameterManipulations/Outputs/Reps_Fear_Weekly_Effort_salmon_eaten_output.csv")
colnames(read.eaten) <- c("Iteration", "Eaten")
...
read.seals.base <- read.csv("ParameterManipulations/Outputs/Reps_Fear_Weekly_Effort_gauntlet_seals_output.csv")
read.seals.base <- read.seals.base[,-2]
colnames(read.seals.base) <- c("Day", 1:times)
seals_for_plot_base <- melt(data = read.seals.base, id.vars = colnames(read.seals.base)[1], 
                            variable.name = "Iteration", value.name = "Gauntlet_Seals")
